...
local RobuxLeaderboardClient = {}
RobuxLeaderboardClient.__index = RobuxLeaderboardClient

local player = Players.LocalPlayer

function RobuxLeaderboardClient.new()
	local self = setmetatable({}, RobuxLeaderboardClient)
	self._connections = {}
...
function RobuxLeaderboardClient:_initialize()
	if RunService:IsClient() then
		self:_setupRemoteEvents()
		self:_createLeaderboardUI()
		self:_requestInitialData()
		
		ClientLogger:Info("RobuxLeaderboardClient initialized successfully")
	end
end

function RobuxLeaderboardClient:_setupRemoteEvents()
	...
	spawn(function()
		...
		self._connections.LeaderboardDataUpdated = leaderboardDataUpdated.OnClientEvent:Connect(function(leaderboardType, data)
			if leaderboardType == "robux" then
				self:_updateLeaderboardDisplay(data)
			end
		end)
		
		ClientLogger:Debug("RobuxLeaderboardClient RemoteEvents connected")
	end)
end

function RobuxLeaderboardClient:_requestInitialData()
	spawn(function()
		wait(1) -- Give time for RemoteEvents to set up
		if self._getLeaderboardData then
			local success, data = pcall(function()
				return self._getLeaderboardData:InvokeServer("robux")
			end)
			
			if success and data then
				self:_updateLeaderboardDisplay(data)
			else
				ClientLogger:Warn("Failed to get initial robux leaderboard data")
			end
		end
	end)
end

function RobuxLeaderboardClient:_createLeaderboardUI()
	-- Wait for MostRobux part to exist
	spawn(function()
		local workspace = game:GetService("Workspace")
		local robuxLeaderboard = workspace:WaitForChild("RobuxLeaderboard", 30)
		if not robuxLeaderboard then
			ClientLogger:Error("RobuxLeaderboard folder not found in workspace")
			return
		end
		
		local mostRobuxPart = robuxLeaderboard:WaitForChild("MostRobux", 30)
		if not mostRobuxPart then
			ClientLogger:Error("MostRobux part not found in RobuxLeaderboard")
			return
		end
		...
		billboardGui.Parent = mostRobuxPart
		
		-- ROBUX title
		local robuxTitle = Instance.new("TextLabel")
		robuxTitle.Name = "RobuxTitle"
		robuxTitle.Size = UDim2.new(1, 0, 0, 60)
		robuxTitle.Position = UDim2.new(0, 0, 0, 0)
		robuxTitle.BackgroundTransparency = 1
		robuxTitle.Text = "ROBUX"
		robuxTitle.TextColor3 = Color3.fromRGB(50, 220, 50)
		robuxTitle.TextSize = 48
		robuxTitle.Font = Enum.Font.FredokaOne
		robuxTitle.TextXAlignment = Enum.TextXAlignment.Center
		robuxTitle.Parent = billboardGui
		
		-- Add stroke to ROBUX title
		local robuxStroke = Instance.new("UIStroke")
		robuxStroke.Color = Color3.fromRGB(0, 0, 0)
		robuxStroke.Thickness = 4
		robuxStroke.Parent = robuxTitle
		...
		surfaceGui.Parent = mostRobuxPart
		self._surfaceGui = surfaceGui
		
		-- ScrollingFrame for leaderboard entries with dark transparent green background
		local scrollFrame = Instance.new("ScrollingFrame")
		scrollFrame.Name = "ScrollFrame"
		scrollFrame.Size = UDim2.new(1, -60, 1, -60) -- Padding from edges
		scrollFrame.Position = UDim2.new(0, 30, 0, 30)
		scrollFrame.BackgroundColor3 = Color3.fromRGB(30, 150, 30) -- Darker green
		scrollFrame.BackgroundTransparency = 0.5 -- Less transparent
		scrollFrame.BorderSizePixel = 0
		scrollFrame.ScrollBarThickness = 8
		scrollFrame.ScrollBarImageColor3 = Color3.fromRGB(30, 150, 30)
		...
		ClientLogger:Debug("Robux Leaderboard UI created successfully")
	end)
end

function RobuxLeaderboardClient:_formatRobux(amount)
...
function RobuxLeaderboardClient:_formatRobuxLong(amount)
...
function RobuxLeaderboardClient:_createLeaderboardEntry(playerData, rank)
	...
	normalElements.amount.Text = self:_formatRobux(playerData.Amount)
	activeElements.fullAmount.Text = self:_formatRobuxLong(playerData.Amount)
	
	-- Apply robux-specific colors and ensure text visibility
	normalElements.amount.TextColor3 = Color3.fromRGB(40, 200, 40)  -- Green robux amount
	normalElements.placement.TextColor3 = Color3.fromRGB(255, 255, 255)  -- White placement
	activeElements.fullAmount.TextColor3 = Color3.fromRGB(40, 200, 40)  -- Green robux amount
	...
	if icon then
		icon.Image = "rbxassetid://15403000023"
	end
...
function RobuxLeaderboardClient:_updateLeaderboardDisplay(data)
	...
	ClientLogger:Debug(string.format("Updated robux leaderboard display with %d players", #self._leaderboardData))
end

function RobuxLeaderboardClient:Cleanup()
	...
	ClientLogger:Info("RobuxLeaderboardClient cleaned up")
end

return RobuxLeaderboardClient
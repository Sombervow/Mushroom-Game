...
local GemsLeaderboardClient = {}
GemsLeaderboardClient.__index = GemsLeaderboardClient

local player = Players.LocalPlayer

function GemsLeaderboardClient.new()
	local self = setmetatable({}, GemsLeaderboardClient)
	self._connections = {}
...
function GemsLeaderboardClient:_initialize()
	if RunService:IsClient() then
		self:_setupRemoteEvents()
		self:_createLeaderboardUI()
		self:_requestInitialData()
		
		ClientLogger:Info("GemsLeaderboardClient initialized successfully")
	end
end

function GemsLeaderboardClient:_setupRemoteEvents()
	...
	spawn(function()
		...
		self._connections.LeaderboardDataUpdated = leaderboardDataUpdated.OnClientEvent:Connect(function(leaderboardType, data)
			if leaderboardType == "gems" then
				self:_updateLeaderboardDisplay(data)
			end
		end)
		
		ClientLogger:Debug("GemsLeaderboardClient RemoteEvents connected")
	end)
end

function GemsLeaderboardClient:_requestInitialData()
	spawn(function()
		wait(1) -- Give time for RemoteEvents to set up
		if self._getLeaderboardData then
			local success, data = pcall(function()
				return self._getLeaderboardData:InvokeServer("gems")
			end)
			
			if success and data then
				self:_updateLeaderboardDisplay(data)
			else
				ClientLogger:Warn("Failed to get initial gems leaderboard data")
			end
		end
	end)
end

function GemsLeaderboardClient:_createLeaderboardUI()
	-- Wait for MostGems part to exist
	spawn(function()
		local workspace = game:GetService("Workspace")
		local gemsLeaderboard = workspace:WaitForChild("GemsLeaderboard", 30)
		if not gemsLeaderboard then
			ClientLogger:Error("GemsLeaderboard folder not found in workspace")
			return
		end
		
		local mostGemsPart = gemsLeaderboard:WaitForChild("MostGems", 30)
		if not mostGemsPart then
			ClientLogger:Error("MostGems part not found in GemsLeaderboard")
			return
		end
		...
		billboardGui.Parent = mostGemsPart
		
		-- GEMS title
		local gemsTitle = Instance.new("TextLabel")
		gemsTitle.Name = "GemsTitle"
		gemsTitle.Size = UDim2.new(1, 0, 0, 60)
		gemsTitle.Position = UDim2.new(0, 0, 0, 0)
		gemsTitle.BackgroundTransparency = 1
		gemsTitle.Text = "GEMS"
		gemsTitle.TextColor3 = Color3.fromRGB(50, 120, 220)
		gemsTitle.TextSize = 48
		gemsTitle.Font = Enum.Font.FredokaOne
		gemsTitle.TextXAlignment = Enum.TextXAlignment.Center
		gemsTitle.Parent = billboardGui
		
		-- Add stroke to GEMS title
		local gemsStroke = Instance.new("UIStroke")
		gemsStroke.Color = Color3.fromRGB(0, 0, 0)
		gemsStroke.Thickness = 4
		gemsStroke.Parent = gemsTitle
		...
		surfaceGui.Parent = mostGemsPart
		self._surfaceGui = surfaceGui
		
		-- ScrollingFrame for leaderboard entries with dark transparent blue background
		local scrollFrame = Instance.new("ScrollingFrame")
		scrollFrame.Name = "ScrollFrame"
		scrollFrame.Size = UDim2.new(1, -60, 1, -60) -- Padding from edges
		scrollFrame.Position = UDim2.new(0, 30, 0, 30)
		scrollFrame.BackgroundColor3 = Color3.fromRGB(30, 80, 150) -- Darker blue
		scrollFrame.BackgroundTransparency = 0.5 -- Less transparent
		scrollFrame.BorderSizePixel = 0
		scrollFrame.ScrollBarThickness = 8
		scrollFrame.ScrollBarImageColor3 = Color3.fromRGB(30, 80, 150)
		...
		ClientLogger:Debug("Gems Leaderboard UI created successfully")
	end)
end

function GemsLeaderboardClient:_formatGems(amount)
...
function GemsLeaderboardClient:_formatGemsLong(amount)
...
function GemsLeaderboardClient:_createLeaderboardEntry(playerData, rank)
	...
	normalElements.amount.Text = self:_formatGems(playerData.Amount)
	activeElements.fullAmount.Text = self:_formatGemsLong(playerData.Amount)
	
	-- Apply gem-specific colors and ensure text visibility
	normalElements.amount.TextColor3 = Color3.fromRGB(0, 242, 255)  -- Cyan gems amount
	normalElements.placement.TextColor3 = Color3.fromRGB(255, 255, 255)  -- White placement
	activeElements.fullAmount.TextColor3 = Color3.fromRGB(0, 242, 255)  -- Cyan gems amount
	...
	if icon then
		icon.Image = "rbxassetid://93098740765317"
	end
...
function GemsLeaderboardClient:_updateLeaderboardDisplay(data)
	...
	ClientLogger:Debug(string.format("Updated gems leaderboard display with %d players", #self._leaderboardData))
end

function GemsLeaderboardClient:Cleanup()
	...
	ClientLogger:Info("GemsLeaderboardClient cleaned up")
end

return GemsLeaderboardClient